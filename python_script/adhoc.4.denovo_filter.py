...
# add info part
filtered_denovo = csv_name.split('.csv')[0] + '.snpindel.csv'
fw = open('temp.txt','w')
...
with open(filtered_denovo,'rU') as f:
    ...
    fw = open('temp.csv','w')
    w = csv.writer(fw)
    w.writerow(head + ['In repetitive regions','75bp_mappability'])
    for line in r:
        variant = dict(zip(head, line))
        chrom, start, end = 'chr' + variant['CHROM'], variant['POS'], variant['POS']
        variant_id = '_'.join([chrom, start, end])
        region,  mappability = 'No', ''
        if variant_id in rep:
            region = 'Yes'
        if variant_id in map_75bp:
            mappability = map_75bp[variant_id]
        w.writerow(line + [region , mappability])
    fw.close() 
shutil.move('temp.csv', filtered_denovo)
os.remove('temp.txt')
os.remove(csv_name.split('.csv')[0] + '.indel.csv')
os.remove(csv_name.split('.csv')[0] + '.snp.csv')
